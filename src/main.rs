...
async fn adaptor(body: String) -> impl Responder {
    ...
    let block_number_hex_str = para
        .captures_iter(&body)
        .nth(0)
        .map(|c| c[1].to_string())
        .unwrap();
    println!("block number: {:#?}", block_number_hex_str);
    let block_number = u64::from_str_radix(&block_number_hex_str, 16).unwrap();
    let req = if let Ok(api_key) = env::var("ETHSCANAPIKEY") {
        println!("api key: {}", api_key);
        let client = reqwest::Client::new();
        client
            .get(&format!(
			"https://api.etherscan.io/api?module=proxy&action=eth_getBlockByNumber&tag=0x{}&boolean=false&apikey={}", block_number_hex_str, api_key)).send()
    } else {
